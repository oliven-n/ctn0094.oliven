...
cap_roc_combined <- paste0(
  "Test-set ROC curves on the held-out 25% split for the winning model (TabPFN)",
  " and the two honorable mentions (LASSO and XGBoost), among CTN-0027/0030/0051",
  " participants (2006–2016). Relapse is the positive class; line-label AUCs are",
  " test-set ROC AUC. Curves are colored in the University of Miami palette."
)
```
...
## Variable Importance (Winning Model)
...
#| fig-cap: !expr cap_roc_combined
#| fig-width: 6.5
#| fig-height: 4.5

# Test-split ROC AUC scalars. Recomputed here so the chunk is self-contained
# regardless of chunk order; identical to the values in tbl-all-metrics.
lasso_test_auc   <- lasso_test_metrics$.estimate[lasso_test_metrics$.metric == "roc_auc"]
xgboost_test_auc <- xgboost_test_metrics$.estimate[xgboost_test_metrics$.metric == "roc_auc"]
tabpfn_test_auc  <- tabpfn_test_metrics$.estimate[tabpfn_test_metrics$.metric == "roc_auc"]
...
dplyr::bind_rows(
  ...
  ggplot2::scale_color_manual(
    values = c("LASSO" = "#FFC72C", "XGBoost" = "#F47321", "TabPFN" = "#005030")
  ) +
  ggplot2::geom_abline(lty = 3, color = "gray60") +
  ggplot2::labs(
    title    = "Test-set ROC curves: winner and honorable mentions",
    subtitle = paste0(
      "LASSO AUC = ",   round(lasso_test_auc, 3),
      "   •   XGBoost AUC = ", round(xgboost_test_auc, 3),
...
Across every method in the all-methods table, **TabPFN attains the highest full-training-set ROC AUC (`r round(tabpfn_train_auc, 3)`)**, and we designate it the winning model on that basis. The choice is not an artifact of in-sample optimism: carried forward unchanged to the held-out 25% test set, TabPFN remains a top performer (test AUC `r round(tabpfn_test_auc, 3)`), and because no tuning occurs, the same fixed pretrained weights produce both the training and test estimates. The combined ROC curves in @fig-roc-combined show TabPFN alongside the two honorable mentions described below.

We highlight two honorable mentions on grounds other than peak training AUC. **LASSO** is the most economical model: it retains competitive discrimination from a drastically sparse predictor set, keeping only `r nrow(lasso_coef)` non-zero coefficients of roughly 107 candidates after the one-standard-error penalty, within `r round(abs(lasso_test_auc - lr_wopv_test_auc), 3)` test-AUC points of the full unpenalized logistic model (detailed in the LASSO sparsity section below). **XGBoost** earns its mention on different grounds: every tuning candidate exhausted the 5,000-tree budget without ever triggering early stopping, so the model had not yet reached its training ceiling — a larger tree budget at these small learning rates would plausibly raise its performance further, headroom the already-converged models lack. In future work we intend to revisit gradient boosting with a larger tree budget and to evaluate alternative boosted-tree implementations (e.g., LightGBM and CatBoost), which differ in their split-finding and regularization strategies and may extract additional signal.
