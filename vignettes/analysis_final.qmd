...
cap_roc_randfor <- "Random forest ROC curve on the held-out 25% test split (CTN-0027/0030/0051 participants, 2006–2016). Relapse is the positive class."
cap_roc_tabpfn  <- "TabPFN ROC curve on the held-out 25% test split (CTN-0027/0030/0051 participants, 2006–2016). Relapse is the positive class."
```
...
bind_cols(
  tabpfn_train_metrics |> dplyr::select(.metric, Train = .estimate),
...
## ROC Curves for Winner(s)

```{r fig-roc-randfor}
#| fig-cap: !expr cap_roc_randfor
#| fig-width: 5
#| fig-height: 3.5
relapse_pred_randfor_test |>
  ...
  ggplot2::labs(
    ...
    subtitle = paste0("Test AUC = ", round(randfor_test_auc, 3)),
...
relapse_pred_tabpfn_test |>
  ...
  ggplot2::labs(
    ...
    subtitle = paste0("Test AUC = ", round(tabpfn_test_auc, 3)),
    x = "1 - Specificity", y = "Sensitivity"
  ) +
  ggplot2::theme_minimal(base_size = 11)
```
...
**TabPFN (pretrained transformer).** *Strengths:* zero tuning overhead — the pretrained weights generalize to new tasks out-of-the-box, so all compute goes to inference rather than search; it matches or exceeds tuned tree-ensemble methods on small tabular datasets in published benchmarks [@hollmann2023], it has no hyperparameters to overfit, and it requires no iterative fitting. *Weaknesses:* it is a black box with no interpretable model form (we resort to permutation importance, as with KNN); the pretrained transformer was trained on synthetic datasets of at most 1,000 samples, so applying it to our ~1,869-row training set requires disabling a safety guard (`ignore_pretraining_limits = TRUE`) and may push beyond the model's reliable operating regime; the Python dependency introduces a cross-language environment requirement that complicates deployment; and permutation importance with a single pass (necessary given TabPFN's inference cost) is noisier than multi-permutation estimates.

Across methods, test ROC AUC remains in a fairly narrow band: the predictive signal is modest and the linear baselines stay competitive, but the tree-ensemble methods and TabPFN achieve the best discrimination. Model choice therefore weighs that edge against the interpretability and stability the simpler models offer.

## Best-Performing Model and Cross-Validation Agreement

Selecting on cross-validated ROC AUC — the criterion fixed in advance — the **best-performing model is `r if (tabpfn_cv_auc >= xgboost_cv_auc) "TabPFN" else "XGBoost"`** (CV AUC `r round(max(tabpfn_cv_auc, xgboost_cv_auc), 3)`), the highest of any method in the all-methods table. Carried forward unchanged to the held-out 25% test set, it remains the strongest model (test AUC `r round(if (tabpfn_cv_auc >= xgboost_cv_auc) tabpfn_test_auc else xgboost_test_auc, 3)`), so the choice made on training-set cross-validation generalizes rather than reflecting an overfit selection. For XGBoost: every tuning candidate exhausted the 5,000-tree budget without triggering early stopping, suggesting a larger tree budget at these small learning rates would plausibly raise XGBoost's performance further — headroom the already-converged models do not have. For TabPFN: no tuning occurred; the single-pass inference means the CV estimate and final performance are produced by the same fixed pretrained weights.
