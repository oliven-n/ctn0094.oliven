...
Despite discarding `r 107 - nrow(lasso_coef)` predictors entirely, the LASSO test ROC AUC (`r round(all_metrics$.estimate[all_metrics$method == "LASSO" & all_metrics$.metric == "roc_auc"], 3)`) is within `r round(abs(all_metrics$.estimate[all_metrics$method=="LASSO" & all_metrics$.metric=="roc_auc"] - all_metrics$.estimate[all_metrics$method=="Logistic (no problem vars)" & all_metrics$.metric=="roc_auc"]), 3)` AUC points of the unpenalized no-problem-variables logistic regression — a negligible practical difference. This suggests the predictive signal in the pre-treatment feature matrix is concentrated in a small subset of predictors; the remaining features either carry redundant information that the L1 penalty zeros out, or add noise that hurts generalization.
