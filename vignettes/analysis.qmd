...
The baseline logistic regression achieved a ROC AUC of `r round(lr_metrics$.estimate[lr_metrics$.metric == "roc_auc"], 3)` on the training set and `r round(lr_test_metrics$.estimate[lr_test_metrics$.metric == "roc_auc"], 3)` on the held-out test set — a mild to moderate drop consistent with overfitting expected from fitting an unregularized model to 100+ predictors, but still reflecting meaningful discriminative ability above chance. An initial model that included `treatment_arm` as a predictor yielded a test-set AUC of approximately 0.693; removing it (to avoid conditioning on post-randomization assignment) reduced test AUC by roughly 0.004, confirming that the feature contributed a small but real signal — likely through its correlation with study site and MOUD eligibility criteria rather than any genuine clinical mechanism. One result that warrants further investigation is the odds ratio for CTN-0030 study membership (OR ≈ 10.5), which is implausibly large and likely reflects a study-level confounder — perhaps differences in outcome ascertainment, follow-up duration, or population composition across trials — rather than a genuine clinical signal. We are not sure why this might be but are looking into it as a next step.
