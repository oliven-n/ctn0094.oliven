...
cap_all_metrics <- paste0(
  "Test-set performance of all fitted models for predicting relapse among the ",
  scales::comma(nrow(analysis_tibble)),
  " care-seeking adults with opioid use disorder in CTN-0027/0030/0051 (2006-2016).",
  " ROC AUC is threshold-free; sensitivity and specificity are reported at the",
  " Youden-J operating point chosen on the training set.",
  " The logistic CV row reports 5-fold cross-validated means."
)
...
*Interpretation (speculative):* The large study effects likely reflect a combination of two mechanisms. First, each trial enrolled a distinct population under different eligibility criteria — CTN-0030 (buprenorphine-naloxone dose study) and CTN-0027 (buprenorphine vs. methadone) enrolled participants with different baseline clinical profiles from CTN-0051 (XR-naltrexone vs. buprenorphine), and those population differences are genuine predictive signals. Second, each study represents a specific treatment ecosystem — protocols, clinical support structures, follow-up design, and staffing — that directly shapes how participants fare. In this sense, `project` acts as a proxy for the entire care environment the participant enters, which is directly related to the interventions and support they receive. The model is therefore not "learning the study label" in a meaningless way; it is learning that care context predicts outcome, which is clinically real.

*Generalizability caveat (speculative):* Because `project` is a coarse proxy, the model as trained may not generalize well to new studies with different protocols or populations. A more generalizable version would encode features of the treatment and clinical setting directly (e.g., medication type, session frequency, treatment duration, site support resources) rather than relying on study-membership dummies. Encoding treatment features rather than the project proxy is a meaningful direction for future model development.
