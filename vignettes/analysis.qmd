...
```{r knn-save-metrics}
#| include: false
# knn_wopv.R overwrites relapse_pred_knn, kknn_tune, favorite — capture KNN
# (non-wopv) train AUC and CV AUC before that happens. # added 6/2
knn_train_auc <- yardstick::roc_auc(
  relapse_pred_knn, truth = outcome, .pred_1, event_level = "second"
)$.estimate
knn_cv_auc <- tune::collect_metrics(kknn_tune) |>
...
# KNN wopv train + CV AUC (captured here since kknn_tune/favorite now = wopv versions)
knn_wopv_train_auc <- yardstick::roc_auc(
  relapse_pred_knn, truth = outcome, .pred_1, event_level = "second"
)$.estimate
knn_wopv_cv_auc <- tune::collect_metrics(kknn_tune) |>
  dplyr::filter(.metric == "roc_auc", neighbors == favorite$neighbors) |>
  dplyr::pull(mean)
...
# CV AUC at selected hyperparameter (from tune objects)
enet_cv_auc  <- tune::collect_metrics(enet_tune) |>
  dplyr::filter(.metric == "roc_auc", penalty == enet_favorite$penalty,
                mixture == enet_favorite$mixture) |> dplyr::pull(mean)
lasso_cv_auc <- tune::collect_metrics(lasso_tune) |>
...
enet_train_auc  <- yardstick::roc_auc(relapse_pred_enet,  truth = outcome, .pred_1, event_level = "second")$.estimate
lasso_train_auc <- yardstick::roc_auc(relapse_pred_lasso, truth = outcome, .pred_1, event_level = "second")$.estimate
...
enet_test_auc    <- round(tune::collect_metrics(enet_last_fit)$.estimate[tune::collect_metrics(enet_last_fit)$.metric == "roc_auc"], 3)
lasso_test_auc   <- round(tune::collect_metrics(lasso_last_fit)$.estimate[tune::collect_metrics(lasso_last_fit)$.metric == "roc_auc"], 3)
